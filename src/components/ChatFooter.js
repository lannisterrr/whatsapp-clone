...
export default function ChatFooter({
  ...
  const showSend = Boolean(input.trim() || image);
  ...
  return (
    <div className="chat__footer">
      <form>
        ...
        {canRecord ? (
          <button
            onClick={showSend ? handleSendMessage : handleStartRecording}
            type="submit"
            className={`send__btn ${showSend ? 'send__btn--send' : 'send__btn--mic'}`}
            disabled={isUploading}
            aria-label={showSend ? 'Send message' : 'Record audio'}
          >
            {showSend ? (
          ...
          <>
            <label htmlFor="capture" className="send__btn send__btn--mic">
              <MicRounded style={{ width: 24, height: 24, color: 'white' }} />
            </label>
            <input
              style={{ display: 'none' }}
