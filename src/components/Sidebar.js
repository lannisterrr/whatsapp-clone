...
export default function Sidebar({ user, page }) {
  ...
  const chats = useChats(user, activeTab === 1);
  const rooms = useRooms(activeTab === 2);
  const users = useUsers(user, activeTab === 3);
    ...
    >
      <div className="sidebar__header">
        <div className="sidebar__header--left">
          <UserAvatar photoURL={user?.photoURL} name={user?.displayName} id={user?.uid} />
